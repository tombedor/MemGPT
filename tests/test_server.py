...
def test_attach_source_to_agent(server, user_id, agent_id):
    ...
    passages_before = server.get_agent_archival(
        user_id=user_id, agent_id=agent_id, start=0, count=10000
    )
    ...
    # check archival memory size
    passages_after = server.get_agent_archival(
        user_id=user_id, agent_id=agent_id, start=0, count=10000
    )
...
def test_get_recall_memory(server, user_id, agent_id):
    ...
    cursor1, messages_1 = server.get_agent_recall_cursor(
        user_id=user_id, agent_id=agent_id, reverse=True, limit=2
    )
    cursor2, messages_2 = server.get_agent_recall_cursor(
        user_id=user_id, agent_id=agent_id, reverse=True, after=cursor1, limit=1000
    )
    cursor3, messages_3 = server.get_agent_recall_cursor(
        user_id=user_id, agent_id=agent_id, reverse=True, limit=1000
    )
    ...
    cursor4, messages_4 = server.get_agent_recall_cursor(
        user_id=user_id, agent_id=agent_id, reverse=True, before=cursor1
    )
    assert len(messages_4) == 1

    # test in-context message ids
    in_context_ids = server.get_in_context_message_ids(
    ...
    # test recall memory
    messages_1 = server.get_agent_messages(
        user_id=user_id, agent_id=agent_id, start=0, count=1
    )
    assert len(messages_1) == 1
    messages_2 = server.get_agent_messages(
        user_id=user_id, agent_id=agent_id, start=1, count=1000
    )
    messages_3 = server.get_agent_messages(
        user_id=user_id, agent_id=agent_id, start=1, count=5
    )
    # not sure exactly how many messages there should be
    assert len(messages_2) > len(messages_3)
    # test safe empty return
    messages_none = server.get_agent_messages(
        user_id=user_id, agent_id=agent_id, start=1000, count=1000
    )
...
def test_get_archival_memory(server, user_id, agent_id):
    ...
    cursor1, passages_1 = server.get_agent_archival_cursor(
        user_id=user_id, agent_id=agent_id, reverse=False, limit=2, order_by="text"
    )
    ...
    passage_1 = server.get_agent_archival(
        user_id=user_id, agent_id=agent_id, start=0, count=1
    )
    assert len(passage_1) == 1
    passage_2 = server.get_agent_archival(
        user_id=user_id, agent_id=agent_id, start=1, count=1000
    )
    assert len(passage_2) == 4
    # test safe empty return
    passage_none = server.get_agent_archival(
        user_id=user_id, agent_id=agent_id, start=1000, count=1000
    )
