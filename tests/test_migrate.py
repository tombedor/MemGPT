...
def test_migrate_0211():
    ...
    try:
        ...
        assert (
            len(agent_res["failed_migrations"]) == 0
        ), f"Failed migrations: {agent_res}"
        ...
        for agent_name in agent_res["migration_candidates"]:
            if agent_name not in agent_res["failed_migrations"]:
                ...
                agent_state = server.ms.get_agent(
                    agent_name=agent_name, user_id=agent_res["user_id"]
                )
                assert agent_state is not None, f"Missing agent {agent_name}"

                # assert in context messages exist
                message_ids = server.get_in_context_message_ids(
                    user_id=agent_res["user_id"], agent_id=agent_state.id
                )
